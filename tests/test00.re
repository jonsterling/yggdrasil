...
let module A = Term.Arity;
let module N = Term.Node;
let module R = Term.Rose;
...
let analyze node => {
  let rose = Inf.Node.arity sg Ctx.empty node;
  let () = {
...
let () = fprintf std_formatter "%a" Computad.pp sg;

let () = {
  let rose = analyze @@ not *@ [pt ff];
  assert (Rose.equal rose @@ pt bool)
};

let () = {
  let rose = analyze @@ con *@ [];
  assert (Rose.equal rose @@ [pt bool, pt bool] --> bool)
};

let () = {
  let rose = analyze @@ con *@ [pt ff];
  assert (Rose.equal rose @@ [pt bool] --> bool)
};

let () = {
  let rose = analyze @@ con *@ [pt ff, pt ff];
  assert (Rose.equal rose @@ pt bool)
};

let () = {
  let rose = analyze @@ map;
  assert (Rose.equal rose @@ [[pt nat] --> nat, pt list] --> list)
};

let () = {
  let rose = analyze @@ map *@ [pt succ];
  assert (Rose.equal rose @@ [pt list] --> list)
};

let () = {
  let rose = analyze @@ map *@ [pt succ, pt nil];
...
let () = {
  let f = N.Lm [("x", pt nat)] (succ *@ [pt @@ N.Var "x"]);
  let _ = analyze @@ map *@ [pt f, pt nil];
  ()
};

let () = {
  let f = N.Lm [("f", [pt nat] --> nat), ("x", pt nat)] (N.Op "succ" *@ [pt @@ N.Var "f" *@ [pt @@ N.Var "x"]]);
  let _ = analyze @@ f;
  ()
};

let () = {
  let f = N.Lm [("f", [pt nat] --> nat)] (N.Lm [("x", pt nat)] (N.Op "succ" *@ [pt @@ N.Var "f" *@ [pt @@ N.Var "x"]]));
  let _ = analyze @@ f;
  ()
};
